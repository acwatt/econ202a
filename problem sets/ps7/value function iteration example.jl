...
function update_J!(J, graph)
    ...
    for node in keys(graph)
        # if end node, always set to 0
        if node == 99
            next_J[node] = 0
        # Otherwise, pick the node that min. cost of travel to next node and the value function at that next node
        else
...
# Initialize J (starting guess)
J = Dict((node => Inf) for node in keys(graph))


# Iterate using the value function until convergence
while true
