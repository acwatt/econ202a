...
\begin{wrapfigure}{r}{0.5\textwidth}
  ...
  \end{center}
  \caption{Production function (top) and marginal product of capital (bottom).}
\end{wrapfigure}

\begin{align*} 
    Y &= \min(AK,BL) = \begin{cases}
        AK & \text{if } AK<BL \\
        BL & \text{if } AK>BL
    \end{cases}\\
    \frac{Y}{L} &= \begin{cases}
        A\dfrac{K}{L} & \text{if } AK<BL \\
        B & \text{if } AK>BL
    \end{cases}\\
    y &= \begin{cases}
        Ak & \text{if } Ak<B \\
...
f(k) &= \begin{cases}
    ...
    \end{cases}
    \intertext{Then}
\frac{sf(k)}{k} &= \begin{cases}
        sA & \text{if } Ak<B \\
        \dfrac{sB}{k} & \text{if } Ak>B
    \end{cases}
\end{align*}
So the plot looks like
\includegraphics[width=0.4\textwidth]{3b}

So if $n+\delta < \frac{sf(k)}{k}$
